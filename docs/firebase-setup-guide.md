...
このガイドでは、Meetraceアプリケーションで使用するFirebaseプロジェクトの設定方法と、必要な環境変数の取得方法を詳しく説明します。
...
1. 「Sign-in method」タブをクリックします。
2. 「匿名」を選択し、「有効にする」をクリックします。
3. 「保存」をクリックします。
4. 「Google」を選択し、「有効にする」をクリックします。
5. プロジェクト名とサポートメールアドレスを設定します。
6. 「保存」をクリックします。

### 2.3. 承認済みドメインの設定

本番環境でデプロイする場合は、承認済みドメインにあなたのドメインを追加してください：

1. 「Sign-in method」タブの「承認済みドメイン」セクションで「ドメインを追加」をクリックします。
2. あなたのアプリケーションのドメインを入力します（例：yourdomain.com）。
3. 「追加」をクリックします。
...
1. 最初は「テストモードで開始」を選択します。
2. 「次へ」をクリックします。
3. **重要**: 後でセキュリティルールを更新する必要があります。プロジェクトルートの `firestore.rules` ファイルを参照してください。
4. 本番環境では必ず認証済みユーザーのみがアクセスできるようにルールを設定してください。
